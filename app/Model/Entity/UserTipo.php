<?php

namespace App\Model\Entity;

use \WilliamCosta\DatabaseManager\Database;
use App\Utils\View;
class UserTipo extends Pessoa{

	
	
	//Método responsavel por cadastrar o usuário no Banco de Dados
	public function cadastrar(){

		//Insere usuário no Banco de Dados
		$this->id=(new Database('tipoUser'))->insert([
				'nome' 		=> $this->nome,
				
		]);
		
		//Sucesso
		return true;
	}
	
	//Método responsavel por atualizar os dados no banco
	public function atualizar(){
		return (new Database('tipoUser'))->update('id = '.$this->id,[
				'nome' 		=> $this->nome,
				
		]);
		
		
	}
	
	//Método responsavel por excluir usuário do banco
	public function excluir(){
		return (new Database('tipoUser'))->delete('id = '.$this->id);
		
		//Sucesso
		return true;
	}
	
	//Método responsavel por retornar uma instancia com base no id
	public static  function getTipoUserById($id){
		return self::getTipoUsers('id = '.$id)->fetchObject(self::class);
		
	}
	
	
	//Método responsavel por retornar Usuários
	public static function getTipoUsers($where = null, $order = null, $limit = null, $fields = '*') {
		return (new Database('tipoUser'))->select($where,$order,$limit,$fields);
	}
	
	//Método responsavel por listar As Tipo no select option
	public static function getSelectTipoUser($id){
	    $resultados = '';
	    $results =  self::getTipoUsers(null,'nome asc',null);
	    //verifica se o id não é nulo e obtém o Procedencia do banco de dados
	    if (!is_null($id)) {
	        $selected = '';
	        while ($ob = $results -> fetchObject(self::class)) {
	            
	            //seleciona a Turma do aluno
	            $ob->id == $id ? $selected = 'selected' : $selected = '';
	            //View de Turmas
	            $resultados .= View::render('pages/selectOption/itemSelect',[
	                'id' => $ob ->id,
	                'nome' => $ob->nome,
	                'selecionado' => $selected
	            ]);
	        }
	        //retorna
	        return $resultados;
	    }else{ //se for nulo, lista todos e seleciona um em branco
	        while ($ob = $results -> fetchObject(self::class)) {
	            $selected = '';
	            $resultados .= View::render('pages/selectOption/itemSelect',[
	                'id' => $ob ->id,
	                'nome' => $ob->nome,
	                'selecionado' => $selected
	            ]);
	        }
	        //retorna a listagem
	        return $resultados;
	    }
	}
	



}